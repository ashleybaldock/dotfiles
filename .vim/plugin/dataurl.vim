...
" TODO - operate on partial lines
" TODO - look for, select and operate on SVG tag
"        cursor is inside of/closest to
"
" (Minimal changes) to turn SVG into a dataurl
"
" See: ~/dotfiles/bin/encodeSVG.js
"
command! -bar -range EncodeSvgToDataUrl
      \ :'<,'>!encodeSVG.js `cat`
...
" TODO - operate on partial lines
" TODO - look for, select and operate on closest encoded data
"
command! -bar -range DecodeURI
      \ :'<,'>!node -e 'console.log(decodeURI(process.argv.slice(1).join(" ")))' -- `cat`

command! -bar -range EncodeURI
      \ :'<,'>!node -e 'console.log(encodeURI(process.argv.slice(1).join(" ")))' -- `cat`

" TODO - operate on partial lines
" TODO - look for, select and operate on closest encoded data
"
command! -bar -range DecodeURIComponent
      \ :'<,'>!node -e 'console.log(decodeURIComponent(process.argv.slice(1).join(" ")))' -- `cat`

command! -bar -range EncodeURIComponent
      \ :'<,'>!node -e 'console.log(encodeURIComponent(process.argv.slice(1).join(" ")))' -- `cat`
