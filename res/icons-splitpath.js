...
const intersectSets = (set1, set2) => {
  ...
  return [...smaller.keys()].reduce((resultSet, key) => larger.has(key) ? resultSet.add(key) : resultSet, new Set());
};
  ...
  const makeHtmlElement = (tagName, attributes) => addAttributesTo(document.createElement(tagName), attributes);

  const makeSvgElement = (tagName, attributes) => addAttributesTo(document.createElementNS('http://www.w3.org/2000/svg', tagName), attributes);

  return (tagName, { attributes } = {}) => {
    if (svgTagNames.has(tagName)) {
      if (ambiguousTagNames.has(tagName)) {
        console.warn(`makeElement: ambiguous tag name '${tagName}' - assuming html`);
      } else {
        return makeSvgElement(tagName, attributes);
      }
    if (!htmlTagNames.has(tagName)) {
      console.warn(`makeElement: unknown tag name '${tagName}' - assuming html`);
    }
  ...
  path.attributes.d.value.split(/(?<=z)(?=M)/g)
    .map((part) => svg.appendChild(((pathGroup) => {
      const pathId = `path_${hashCode(part)}`;
      const clipPathId = `clip_${hashCode(part)}`;
      pathGroup.appendChild(makeElement('path', {
        attributes: {
          'd': part,
          'id': pathId,
  ...
  const [x, y, w, h] = svg.attributes.viewBox.value.split(' ').map((i) => parseInt(i, 10));
  
  const scale = 0.8;
  
  svg.setAttribute('viewBox', `${(((w / scale) - w) * -0.5).toFixed(2)} ${(((h / scale) - h) * -0.5).toFixed(2)} ${(w / scale).toFixed(2)} ${(h / scale).toFixed(2)}`)
  
  parentSvg.parentNode.appendChild(svg);
}


document.querySelectorAll('svg > path:first-child').forEach((path) => splitpath(path))
