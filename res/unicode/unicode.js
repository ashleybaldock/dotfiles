...
const genCodePoints = (start, end, chunk = 8) =>
 [...rangeIter({start, end})]
  .map((codePoint, i) =>
      `<a>&#x${codePoint.toString(16).padStart(4, '0')};</a>${(i+1) % chunk
       ? '' 
       : `<!--${(i+1) % (chunk * 2) ? '' : `0x${i.toString(16).padStart(4, '0')}`}\n-->`}`);

  const result = [...blocks.entries()].slice(0, 32).map(([name, [start, end]]) => 
   `<dt data-start="${start}" data-end="${end}" data-count="${end-start+1}" data-name="${name}">${name}</dt>
<dd><!-- ${`0x${start.toString(16).padStart(4, '0')}-0x${(end).toString(16).padStart(4, '0')} (${end-start+1})`}
-->${genCodePoints(start,end).join('')}
</dd>
`).join('\n\n')
  return result;
})()
